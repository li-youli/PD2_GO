...
Node* insertAtHead(Node* head, float value) {
...
void findExtremes(Node* head) {
    if (head == NULL) {
        printf("[ERROR] Cannot find extremes in an empty list.\n");
        return;
    }
...
void searchNode(Node* head, int total_nodes) {
    if (head == NULL) {
        printf("[ERROR] The list is empty. Nothing to search.\n");
        return;
    }
...
int main() {
    ...
    while (1) {
        ...
        if (menu == 1) {
            ...
            printf("\nShutting down system... Freeing RAM.\n");
            freeList(head);
            head = NULL; 
